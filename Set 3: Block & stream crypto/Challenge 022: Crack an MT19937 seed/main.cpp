...
{
    time_t seed = time(0);
    time_t cracked_seed = 0;
    // seed = 0;
    seed_mt(seed);
    mt19937 mt_rand(seed);
    unsigned output0 = mt_rand();
    cout<<"seed: "<<int(seed)<<endl;
    cout<<"output0: "<<output0<<endl;

    const int seed_start = 1577952288;
    const int seed_end = time(0);
    cout<<"seed_end: "<<int(seed)<<endl;
    for (int i = seed_end; i >= 0; --i) {
        seed_mt(i);
        unsigned output = extract_number();
        if (output == output0) {
            cracked_seed = i;
            cout<<"total iterations: "<<seed_end - i + 1<<endl;
            cout<<"cracked_seed: "<<int(seed)<<endl;
            break;
        }
    }
    return 0;
